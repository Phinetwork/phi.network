...
import { useSigilSession } from "./session/useSigilSession";
  ...
  function useDraft(key: string, initial: Draft) {
    ...
    useEffect(() => {
      try {
        ...
        /* non-fatal: ignore quota or private mode */
      }
    }, [key, draft]);
  ...
  function useKaiTicker(): KaiMoment {
    ...
    useEffect(() => {
      ...
      // Align the first tick to the next pulse boundary for smoothness
      const tick = () => {
        if (!alive) return;
        setNow(getKaiNow());
  ...
  export default function SigilPublisherPanel(props: { appGlyphUrl?: string }) {
    ...
    // NEW: simple export (so the modal can “download” something immediately)
          ...
          try {
            ...
            if (typeof window !== "undefined") {
              window.dispatchEvent(
                new CustomEvent("sigil:published", { detail: { url } })
              );
            }
          } catch (e: unknown) {
    ...
    const disabledReason = useMemo(() => {
      ...
      if (session.expiresAtPulse && moment.pulse > session.expiresAtPulse)
        return "Session expired; re-login.";
      return "";
            ...
            <div className="ks-head-right">
              ...
              >
                <div className="ks-pulse-bar">
                  <div
                    className="ks-pulse-fill"
                    style={{ width: `${pulsePct.toFixed(1)}%` }}
                  />
                </div>
